...
### Split Test Files (Completed)
- `ValidSelectQueriesTest.java` — 17 positive test cases
- `BasicDmlDdlRejectionTest.java` — 18 basic DML/DDL rejection tests
- `CTEDMLTest.java` — 3 CTE-with-DML tests
...
**Layer 1: String-Based Pre-checks** (Fast, catches unparseable patterns)
- SELECT INTO detection (with quote-aware filtering)
- Index hints (USE/FORCE/IGNORE INDEX)
- Sequence functions (nextval, setval, LAST_INSERT_ID, NEXTVAL, NEXT VALUE FOR, IDENTITY)
- Stored procedure/function calls (heuristic name matching)
- CTE-embedded DML detection (DELETE/INSERT/UPDATE/MERGE/TRUNCATE in WITH clauses)

**Layer 2: JSqlParser AST** (Robust, full parsing)
- Statement type validation (only `Select` instances allowed)
- Multi-statement detection and per-statement validation
- CTE body inspection for DML statements
- State-modifying function detection within SELECT (nextval, setval, uuid_generate_v1)

**Layer 3: Fail-Safe** (Conservative)
- Parse failures treated as invalid — when in doubt, reject
- Descriptive error messages with specific violation details
- Supports LLM self-correction via actionable rejection reasons
...
1. **COMPREHENSIVE_TEST_SUITE_SUMMARY.md** — Full project test suite overview
2. **This document** — SQL validation executive summary
